...
**当前状态：** HEADLESS_WORKER_READY + **WOL_SELFTEST_PASS** + **COLD_MIGRATE_COPY_DONE**（Mac 原件未删）+ **WATCHDOG_USERSAFE_DEPLOYED**（运行中，待本仓 push 与运行副本对齐）

**更新时间：** 2026-08-09 11:30 CST（Mac agent 现场复核）
...
2. **P1 — SSH 间歇 `kex_exchange_identification: Connection closed by remote host`（无 banner）**  
   ...
   - 现状：此刻 SSH **可用**。需在失败时抓 Win 事件/sshd 日志，勿空转改 Mac 侧。
...
| 角色 | 现在做 | 不要做 |
|---|---|---|
| **Mac agent** | 推送本仓（SHARED_STATUS + jobs 看门狗与运行副本一致）；用 SSH 做电源/连通/日志复核；编排 Cloud 重活 | 勿再改 Desktop iCloud 鬼目录；勿用 ping 判死 |
| **Windows Cursor** | `git pull` 本仓读本文件；**核对** `C:\FactorOS\jobs` 与仓库一致（或按 `install_watchdog.ps1` 重装任务）；用本机 UI 验「询问/取消」；可选配对 Bluetooth；本机看 Event Viewer / sshd 当 SSH 再挂 | 勿改 Mac 编排真相仓当唯一源；勿把 fleet/LLM 重活塞进 Win |
| **用户** | Mac 用 Windows App 连 RDP；需要时长开时 `win_ctl.sh keepalive`；确认后再 purge 冷数据 | 勿同时让两边互相覆盖同一未提交文件而不 pull |
...
| 项 | 值 |
|---|---|
| SSH | `ssh factoros-win` OK（开机后复核；仍可能间歇无 banner） |
...
| 脚本部署 | 运行副本：`C:\FactorOS\jobs\`（任务指向此处） |
